...
use assemblyline_models::datastore::submission::{File, SubmissionParams};
...
impl Ingest {
    ...
    pub async fn get_message(&self, nq: &str) -> Result<Option<IngestMessage>, Error> {
        let url = self.connection.get_server_path(&api_path!(INGEST_PATH, "get_message", nq))?;
    ...
    pub async fn get_message_list(&self, nq: &str) -> Result<Vec<IngestMessage>, Error> {
        let url = self.connection.get_server_path(&api_path!(INGEST_PATH, "get_message_list", nq))?;
        return Ok(self.connection.get(url, None, convert_api_output_obj).await?);
    }
}
...
impl IngestBuilder {
    ...
    pub fn notification_queue(mut self, name: String) -> Self {
        self.notification_queue = Some(name);
        self
    }
    pub fn notification_threshold(mut self, threshold: i64) -> Self {
        self.notification_threshold = Some(threshold);
        self
    }
...
impl NamedIngestBuilder {
    ...
    async fn submit_file(self, body: reqwest::Body) -> Result<IngestResponse, Error> {
        ...
        // println!("{multipart:?}");
        let url = self.parent.connection.get_server_path(&api_path!(INGEST_PATH))?;
        return Ok(self
            .parent
            .connection
