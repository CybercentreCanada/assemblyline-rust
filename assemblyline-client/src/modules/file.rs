...
use assemblyline_utilities::connection::{convert_api_output_obj, convert_api_output_string, convert_output_stream, Connection};
use serde::Deserialize;

use crate::types::{IBool, Result};
...
impl File {
    ...
    pub async fn ascii(&self, hash: &Sha256) -> Result<String> {
        let url = self.connection.get_server_path(&api_path!(FILE_INDEX, "ascii", hash))?;
    ...
    pub async fn children(&self, sha256: &Sha256) -> Result<Vec<Child>> {
        let url = self.connection.get_server_path(&api_path!(FILE_INDEX, "children", sha256))?;
    ...
    pub async fn download(
        ...
        let url = self.connection.get_server_path(&api_path!(FILE_INDEX, "download", sha256))?;
    ...
    pub async fn hex(&self, sha256: &Sha256, bytes_only: impl IBool, length: Option<usize>) -> Result<String> {
        ...
        let url = self.connection.get_server_path(&api_path!(FILE_INDEX, "hex", sha256))?;
        Ok(self.connection.get_params(url, params, None, convert_api_output_string).await?)
    }
    ...
    pub async fn info(&self, sha256: &Sha256) -> Result<FileModel> {
        let url = self.connection.get_server_path(&api_path!(FILE_INDEX, "info", sha256))?;
        return Ok(self.connection.get(url, None, convert_api_output_obj).await?);
    }
    ...
    pub async fn result(&self, sha256: &Sha256) -> Result<FileResults> {
        let url = self.connection.get_server_path(&api_path!(FILE_INDEX, "result", sha256))?;
        return Ok(self.connection.get(url, None, convert_api_output_obj).await?);
    }
    ...
    pub async fn result_for_service(&self, sha256: &Sha256, service: &str) -> Result<FileResultForService> {
        let url = self.connection.get_server_path(&api_path!(FILE_INDEX, "result", sha256, service))?;
        return Ok(self.connection.get(url, None, convert_api_output_obj).await?);
    }
    ...
    pub async fn score(&self, sha256: &Sha256) -> Result<FileScore> {
        let url = self.connection.get_server_path(&api_path!(FILE_INDEX, "score", sha256))?;
        return Ok(self.connection.get(url, None, convert_api_output_obj).await?);
    }
    ...
    pub async fn strings(&self, sha256: &Sha256) -> Result<impl futures::Stream> {
        let url = self.connection.get_server_path(&api_path!(FILE_INDEX, "strings", sha256))?;
        return Ok(self.connection.get(url, None, convert_output_stream).await?);
    }
}
