...
use crate::{
    ...
    constants::{
        get_version, DEFAULT_SERVICE_API_KEY, DONE_FIFO_NAME, EXCEPTION_SERVICE_ERROR_TYPE,
        PLACEHOLDER_VERSION_TAG, SUPPORTED_API, TASK_DONE_ERROR, TASK_DONE_SUCCESS, TASK_FIFO_NAME,
        UNRECOVERABLE_ERROR_STATUS,
    },
...
impl ServiceClient {
    pub async fn new(
        ...
        info!(
            "Service: {} Version: {}",
            service_manifest.service.name, service_manifest.service.version
        );
        ...
        headers.insert(
            ...
            service_manifest.service.name.to_string(),
        );
        headers.insert(
            "Service-Version".to_string(),
            service_manifest.service.version.clone(),
        );
        headers.insert(
    ...
    async fn update_client_header(&mut self) -> Result<()> {
        ...
        headers.insert(
            "Service-Tool-Version".to_string(),
            self.tool_version.clone().unwrap_or("".to_string()),
        );

        self.connection.update_client(headers).await?;
        Ok(())
    }
    ...
    async fn _setup_fifo_pipes(&self) -> Result<TaskFifoPipes> {
        ...
        debug!(
            "Trying to make fifo pipe at {} and at {}",
            &self.task_fifo_path, &self.done_fifo_path
        );
        ...
        unsafe {
            ...
            if status_code_task != 0 {
                return Err(anyhow!(
                    "Cannot create task named queue at {}",
                    self.task_fifo_path
                ));
            }
            ...
            if status_code_done != 0 {
                return Err(anyhow!(
                    "Cannot create done named queue at {}",
                    self.done_fifo_path
                ));
            }
        };
    ...
    pub async fn register_service(&mut self) -> Result<bool, ServiceHandlerError> {
        let register_url = self
            .connection
            .get_api_path(SUPPORTED_API, "service", &["register"])?;
        ...
        let data: APIResponse<RegisterResponse> =
            response.json::<APIResponse<RegisterResponse>>().await?;

        if !data.api_response.new_heuristics.is_empty() {
            info!(
                "New heuristics registered: {}",
                data.api_response.new_heuristics.join(", ")
            );
        }

        // load new service configuration
        self.service = data.api_response.service_config.to_owned();
    ...
    async fn process_task_done_message(
        ...
        let done_file_path = done_message.get(0).ok_or(ServiceHandlerError::Default(
            "Cannot find result file path in message from service.".to_string(),
        ))?;

        info!(
            "[{}]-sid[{}] Task done status is: {:?}",
            &task.task_id,
            &task.sid,
            done_message.get(1)
        );
        ...
        match done_message.get(1) {
            Some(status) => {
                match status.as_str() {
                    TASK_DONE_SUCCESS => {
                        ...
                        let task_done_result: service_api::result::Result =
                            serde_json::from_slice(data.as_slice())?;

                        // update service tool version if there is a change
                        if let Some(service_tool_version) =
                            &task_done_result.response.service_tool_version
                        {
                            if self
                                .tool_version
                                .as_ref()
                                .is_some_and(|version| *service_tool_version != *version)
                            {
                        ...
                        let result = task_uploader
                            .upload_task_result(&task, task_done_result, &self.connection)
                            .await;
                        ...
                        match result {
                            Ok(_) => info!(
                                "[{}]-sid[{}] Successfully upload task results.",
                                &task.task_id, &task.sid
                            ),
                            Err(err) => {
                                // upload this error
                                warn!(
                                    "[{}]-sid[{}] Error uploading task results. {}",
                                    &task.task_id, &task.sid, err
                                );
                                let _ = task_uploader
                                    .upload_task_error(
                                        &task,
                    ...
                    _ => {
                        return Err(ServiceHandlerError::Default(format!(
                            "Unknown task done status {}",
                            status
                        )))
                    }
                }
            ...
            None => {
                return Err(ServiceHandlerError::Default(format!(
                    "Did not get task done status from service."
                )))
            }
        };
    ...
    async fn write_task_to_fifo(
        ...
        let data = [
            task_message_data.to_string().as_bytes(),
            "\n".as_utf8_bytes(),
        ]
        .concat();
        fifo_pipes
            ...
            .await
            .map_err(|e| ServiceHandlerError::PipeWriteError {
    ...
    async fn read_result_from_fifo(
        &self,
        fifo_pipes: &mut TaskFifoPipes,
        service_process: &mut Child,
    ) -> Result<Vec<String>, ServiceHandlerError> {
        ...
        tokio::select! {
            ...
            status_code = service_process.wait() => {
                ...
                return Err(ServiceHandlerError::PipeWriteError { pipe_name: DONE_FIFO_NAME.to_owned(), message: format!("Service process terminated with status code: {:?}", status_code) });
            }
        }
            ...
            match fifo_pipes.done_fifo.try_read(&mut msg) {
                Ok(data_size) => {
                    ...
                    match value {
                        ...
                        Err(err) => {
                            return Err(ServiceHandlerError::PipeWriteError {
                                pipe_name: DONE_FIFO_NAME.to_owned(),
                                message: format!(
                                    "Unknown message format received from done fifo. Err: {}",
                                    err
                                ),
                            });
                        }
        ...
        Err(ServiceHandlerError::Default(
    ...
    pub async fn run_service(
        ...
        let mut service_process = service_launcher
            .launch_service()
            .await
            .map_err(|e| anyhow!(e))?;
        ...
        while self.is_running() && is_service_running {
            ...
            match task_fetched {
                Ok(None) => {
                    debug!("Cannot find a task.... sleep for 1 seconds");
                    tokio::time::sleep(tokio::time::Duration::from_secs_f64(1.0)).await;
                }

                Err(err) => {
                    log_error!("Error fetching task: {}", err);
                    debug!("Sleep for 1 seconds...");
                    tokio::time::sleep(tokio::time::Duration::from_secs_f64(1.0)).await;
                }
                ...
                Ok(Some(task)) => {
                    ...
                    let write_task_fifo_result = self
                        .write_task_to_fifo(&task_dir_path, &task_file_path, &mut fifo_pipes)
                        .await;

                    if let Some(err) = write_task_fifo_result.err() {
                        log_error!(
                            "Error writing to task fifo. Terminating service handler. {:?}",
                            err
                        );
                        let _ = task_uploader
                            .upload_task_error(
                    ...
                    let read_done_fifo_result = self
                        .read_result_from_fifo(&mut fifo_pipes, &mut service_process)
                        .await;

                    match read_done_fifo_result {
                        Ok(done_message) => {
                            let process_result = self
                                .process_task_done_message(
                                    done_message,
                                    task.clone(),
                                    &task_uploader,
                                )
