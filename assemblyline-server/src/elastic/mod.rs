...
impl Elastic {
    ...
    async fn setup(helper: Arc<ElasticHelper>, prefix: &str) -> Result<Arc<Self>> {
        macro_rules! collection {
            ($name: expr) => {
                Collection::new(helper.clone(), $name.to_owned(), None, prefix.to_string()).await?
            };
            (archive, $name: expr) => {
                Collection::new(helper.clone(), $name.to_owned(), Some($name.to_owned() + "-ma"), prefix.to_string()).await?
            };
        }
