...
// pub fn into_safe_str(s: &[u8]) -> String {
...
//         regex::bytes::Regex::new("(([\x09\x0a\x0d\x20-\x7e]|[\u{c2}-\u{df}][\u{80}-\u{bf}]|[\u{e0}][\u{a0}-\u{bf}][\u{80}-\u{bf}]|[\u{e1}-\u{ec}][\u{80}-\u{bf}]{2}|[\u{ed}][\u{80}-\u{9f}][\u{80}-\u{bf}]|[\u{ee}-\u{ef}][\u{80}-\u{bf}]{2}|[\u{f0}][\u{90}-\u{bf}][\u{80}-\u{bf}]{2}|[\u{f1}-\u{f3}][\u{80}-\u{bf}]{3}|[\u{f4}][\u{80}-\u{8f}][\u{80}-\u{bf}]{2})+)").unwrap()
//     })
// }
...
mod test {
    ...
    // #[test]
    // fn test_safe_str() {
    //     assert_eq!(safe_str("abc123"), "abc123");
